...
describe('YarnLockfile', () => {
  ...
  it('should load and serialize a legacy lockfile', async () => {
    ...
    const lockfile = await YarnLockfile.load(mockDir.resolve('yarn.lock'));
...
describe('YarnLockfile advanced', () => {
  describe('diff', () => {
...
`);
    ...
    it('should reject diffing with a lockfile that is not a yarn lockfile', () => {
      const lockfile = YarnLockfile.parse('');
      const other = {
        get: () => undefined,
      ...
      expect(() => lockfile.diff(other)).toThrow(
